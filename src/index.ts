...
} = require("../build/Release/speex.node");
...
export class SpeexResampler {
  ...
  private readonly _resampler: unknown;
   ...
   */
  constructor(channels: number, inRate: number, outRate: number, quality = 7) {
    if (channels < 1) throw new Error("Channels must be >= 1");
    this._channels = channels;
    this._resampler = nativeSpeex.createResampler(
      channels,
      inRate,
      outRate,
      quality
   ...
   */
  async processChunk(pcmData: Buffer): Promise<Buffer> {
    if (this._processing) {
      throw new Error(
        "Concurrent processing not allowed. Wait for the previous chunk to finish."
      );
    }
    ...
    if (pcmData.length % (this._channels * 2) !== 0) {
      ...
      throw new Error(
        "Chunk length must be multiple of channels × 2 bytes (16-bit PCM alignment)."
      );
    }

    return new Promise<Buffer>((resolve, reject) => {
      nativeSpeex.resampleChunk(
        this._resampler,
        pcmData,
        this._channels,
        (err, buf) => {
          ...
          if (!buf)
            return reject(
              new Error("No output buffer returned from native module.")
            );
          resolve(buf);
        }
      );
...
export class SpeexResamplerTransform extends Transform {
  private readonly resampler: SpeexResampler;
  private readonly channels: number;
  private readonly _alignmentBuffer: Buffer;
  private _alignmentBufferLength = 0;
  ...
  async _transform(
    ...
    if (extraneousBytesCount !== 0) {
      ...
      chunkToProcess.copy(
        this._alignmentBuffer,
        0,
        chunkToProcess.length - extraneousBytesCount
      );
      this._alignmentBufferLength = extraneousBytesCount;
      chunkToProcess = chunkToProcess.subarray(
        0,
        chunkToProcess.length - extraneousBytesCount
      );
    }
